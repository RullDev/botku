...
import { groupResponse } from './events/group_participants.js';

const logger = pino({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({ class: 'irul' });
logger.level = 'fatal';
...
version,
  ...
  markOnlineOnConnect: true
    ...
    hisoka.ev.on('chats.set', () => {

		console.log('got chats', store.chats.all().length)

	})
    
    hisoka.ev.on('contacts.set', () => {
    ...
    try {
        await groupResponse(hisoka, anu);
    } catch (err) {
